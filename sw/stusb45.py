...
class STUSB4500(object):
	...
	def print_pdo(self):
		for k, v in self.read_pdo().iteritems():
			print("PDO#" + str(k) + ": ", v)
	...
	def set_pdo(self, num, volt, current):
		if num > 0 and num < 4:
			...
			self.bus.write_byte_data(self.addr, 0x86 + (num - 1) * 4, (reg32 >> 8) & 0xff)
			# bus.write_byte_data(self.addr, 0x87 + (num - 1) * 4, (reg32 >> 16) & 0xff)
			# bus.write_byte_data(self.addr, 0x88 + (num - 1) * 4, (reg32 >> 24) & 0xff)
		else:
	...
	def set_pdo_variable(self, pdo_num, current, min_voltage, max_voltage):
		if pdo_num > 1 and pdo_num <= 3:
			if min_voltage >= 5000 and min_voltage <= 20000 and max_voltage >= 5000 and max_voltage <= 20000 and min_voltage <= max_voltage:
				...
				self.bus.write_byte_data(self.addr, 0x85 + (pdo_num - 1) * 4, reg32 & 0xff)
				self.bus.write_byte_data(self.addr, 0x86 + (pdo_num - 1) * 4, (reg32 >> 8) & 0xff)
				self.bus.write_byte_data(self.addr, 0x87 + (pdo_num - 1) * 4, (reg32 >> 16) & 0xff)
				self.bus.write_byte_data(self.addr, 0x88 + (pdo_num - 1) * 4, (reg32 >> 24) & 0xff)
		elif pdo_num == 1:
	...
	def nvm_dump(self):
		...
		for num_sector in range(0, 5):
			...
			# execute command
			self.bus.write_byte_data(self.addr, 0x96, (num_sector & 0x07) | 0x80 | 0x40 | 0x10)
	...
	#	Erase specified sectors to be able to program them
		#bus.write_byte_data(self.addr, 0x53, 0x00)
		#bus.write_byte_data(self.addr, 0x96, pwr | rst_n)
		self.bus.write_byte_data(self.addr, 0x97, section_mask << 3 | 0x02) # WRITE_SER opcode
...
if __name__ == "__main__":
	...
	parser.add_argument("-b", "--bus", type=int, default=1, help="I2C bus number. Default is 1")
	parser.add_argument("-a", "--address", type=hex_int, default=0x28, help="Device I2C address. Default is 0x28")
	commands = parser.add_subparsers(help="Command")
	nvm_parser = commands.add_parser("nvm")
	nvm_parser.add_argument("--dump", action="store_true")
	nvm_parser.set_defaults(func=command_nvm)
